...
<img src="https://img.shields.io/badge/Version-0.8.17-green" alt="Version" />
...
Kanbox 是一款面向 macOS 的本地优先资料库，用来把散落在小红书里的图文和视频收藏，整理成真正可以搜索、阅读和再次利用的个人知识资料。你可以拖入链接、批量粘贴，或使用随应用提供的 Chrome 扩展快速收录；Kanbox 会保存正文与媒体，在本机完成图片 OCR、视频文稿整理和自动分类。

收录后的内容可以按分类、标签、收藏和阅读状态管理，并支持字段搜索、批量整理、每日回顾、稍后阅读与最近阅读。图片和视频会进入本地资料目录，完整 `.kanbox` 归档、自动快照、资料库发现与完整性修复用于换机、升级或异常中断后的恢复。

**Kanbox 不读取你的账号 Cookie，也不会自动抓取收藏夹。** 每次导入都由你主动触发，只解析你提供的单篇公开笔记；批量模式最多处理 50 条链接。数据默认留在自己的电脑上，也可以将资料目录切换到 iCloud。跨设备数据按修订号合并，出现同一版本的不同修改时会保留当前合并结果并提示人工确认。

AI 摘要、知识拓展和增强转写属于可选能力：不配置在线模型时仍可使用本地整理功能；配置后，请求只发送到你选择的服务商。

### 关于“同步冲突”

“同步冲突”表示同一条笔记出现了修订号相同、内容却不同的两个副本。它可能来自两台设备同时修改，也可能发生在切换 iCloud 资料目录、恢复旧快照或合并 `.kanbox` 归档时，并不表示 Kanbox 正在使用独立的云端同步服务。

Kanbox 会先确定性合并双方内容并列出发生分叉的字段。请打开对应笔记，检查标题、正文、分类、标签或视频文稿，必要时先完成编辑，再点击“确认当前内容”。确认成功后会保留当前版本、推进修订号并清除冲突标记；如果另一台设备仍开着旧资料，请先退出该设备上的 Kanbox，等待 iCloud 文件同步完成后再确认，避免旧进程重新写入过期副本。

## 🆕 v0.8.17 更新

- **窗口缩放稳定** — 顶部改为有明确宽度边界的网格布局，缩放窗口不再挤压按钮或造成文字换行
- **工具栏安全滚动** — 空间不足时横向滚动且所有按钮均可到达，宽屏下继续保持右侧对齐
- **冲突确认即时刷新** — 修订号、冲突字段和阅读状态纳入刷新判定，确认后不再残留旧提示
- **保存反馈明确** — 冲突确认增加处理中与成功反馈，避免重复点击或误以为没有生效
- **名称更准确** — 界面将“同步冲突”调整为“资料合并冲突”，覆盖跨设备、资料恢复和目录迁移等真实来源
- **README 更新** — 重写软件简介并补充本地优先、资料恢复、可选 AI 与合并冲突说明
- **版本号对齐** — 桌面应用、浏览器扩展、备份、完整归档与安装包统一升至 v0.8.17

## 🆕 v0.8.16 更新

- **筛选层级重整** — 状态视图与内容分类拆分为两条独立筛选栏，不再混排和随机换行
- **控件规格统一** — 筛选按钮统一高度、间距、圆角与选中反馈，信息层级更容易扫读
- **窄窗口适配** — 筛选项超出可用宽度时改为横向滚动，避免挤压主界面
- **顶部空间优化** — 搜索框使用响应式宽度，主要操作按钮间距更紧凑
- **版本号对齐** — 桌面应用、浏览器扩展、完整归档清单与安装包统一升至 v0.8.16
